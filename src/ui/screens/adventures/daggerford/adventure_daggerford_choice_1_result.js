...
export function renderAdventureDaggerfordChoice1Result(root, params){
  ...
  if(choice === 'return'){
    const curran = el('img',{src:'assets/curran.png', alt:'Curran', style:'position:absolute;left:38%;bottom:-220%;z-index:7;max-height:50vh;width:auto;opacity:0;transition:opacity 2000ms ease;pointer-events:none'});
...
The city moves on, indifferent as ever.`;
  ...
  actionBtn.addEventListener('click', ()=>{
    ...
    try{ navigate('adventure_daggerford_scene_2', { ctx }); }catch(e){ try{ navigate('battle', ctx); }catch(e){ /* ignore */ } }
  });
